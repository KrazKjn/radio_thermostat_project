    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                ...
                if (json) {
                    const heatRuntime = Math.round(json.filter(d => d.HVAC && d.HVAC.tmode === HVAC_MODE_HEAT).reduce((acc, cur) => acc + cur.HVAC.total_runtime_minutes, 0) * 100) / 100;
                    const coolRuntime = Math.round(json.filter(d => d.HVAC && d.HVAC.tmode === HVAC_MODE_COOL).reduce((acc, cur) => acc + cur.HVAC.total_runtime_minutes, 0) * 100) / 100;
